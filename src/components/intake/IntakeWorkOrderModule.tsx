                  ...
                  <div>
                    <div className="mb-2 flex flex-wrap items-center gap-1.5">
                      {selectedWorkOrder.priority === 'Urgent' && (
                        <span className="inline-flex h-5 items-center gap-1.5 rounded-md border border-rose-200 bg-rose-50 px-2 text-xs font-extrabold uppercase tracking-[0.08em] text-rose-700">
                          <span className="h-1.5 w-1.5 rounded-full bg-rose-500" />
                          Urgent
                        </span>
                      )}
                      <span className="inline-flex h-5 items-center gap-1.5 rounded-md border border-[var(--border)] bg-[var(--card-bg)] px-2 text-xs font-extrabold uppercase tracking-[0.08em] text-[var(--text-muted)]">
                          ...
                          >
                            <div className="min-w-0">
                              ...
                              {(afterItem.note || beforeItem.note) && (
                                <p className="mt-0.5 max-w-32 truncate pl-5 text-xs italic text-[var(--text-muted)]">
                                  {afterItem.note || beforeItem.note}
                                </p>
                              )}
                            </div>
                            <div className="flex shrink-0 items-center gap-1">
                              <span className={`inline-flex min-w-14 justify-center rounded-md border px-2 py-1 text-xs font-extrabold uppercase ${statusClass(beforeItem.status)}`}>
                                {beforeItem.status}
                              </span>
                              <span className="text-xs font-bold text-[var(--text-muted)]">→</span>
                              <span className={`inline-flex min-w-14 justify-center rounded-md border px-2 py-1 text-xs font-extrabold uppercase ${statusClass(afterItem.status)}`}>
                                {afterItem.status}
                              </span>
                            </div>
                          </div>

    ...
    <div className="space-y-3">
      ...
      <div className="bg-gradient-to-r from-amber-500/10 via-amber-50/60 to-orange-500/10 border border-amber-300/60 p-5 rounded-2xl shadow-2xs">
        <div className="flex items-start justify-between gap-3 flex-wrap">
          ...
          {totalShopCompleted > 0 && (
            <span className="text-xs text-[#86868B] font-semibold bg-white/70 px-3 py-1.5 rounded-xl border border-amber-200 shadow-2xs shrink-0">
      ...
      <div className="bg-white border border-[#E5E5EA] rounded-2xl p-5 shadow-2xs space-y-4">
        ...
        {leaderboardData.length === 0 ? (
          <div className="p-8 text-center text-xs text-[#86868B] bg-[#F8F9FA] rounded-xl border border-dashed border-[#D2D2D7] space-y-1">
            ...
            <p className="font-extrabold text-sm text-[#1D1D1F]">No technicians yet</p>
                    ...
                    >
                      ...
                      <td className="py-3 px-3">
                        <div className="flex items-center space-x-2.5">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-[#0071E3] font-bold hidden md:block">{item.tech.level}</p>
                          </div>
                        </div>
                      </td>

                      <td className="py-3 px-3 text-center">
                        <span className="font-black text-[#1D1D1F] text-sm">{item.liveCompleted}</span>
                        <span className="text-[10px] text-emerald-600 font-bold block">completed</span>
                      </td>

                      <td className="py-3 px-3 text-center hidden sm:table-cell">
                        <span className="text-[10px] font-bold text-[#86868B] bg-[#F5F5F7] px-2 py-0.5 rounded-full border border-[#E5E5EA]">
                      ...
                      <td className="py-3 px-3 text-center">
                        <div className="space-y-0.5">
                          ...
                          <span className="text-[10px] text-[#86868B] font-medium block">labor only</span>
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-3 text-center hidden md:table-cell">
                        {hasRate ? (
                          ...
                          <span className="text-[10px] text-[#86868B] font-medium">— no rate</span>
                        )}
                      </td>
                      ...
                      <td className="py-3 px-3 text-center font-bold">
                        {item.successRate !== null ? (
                          <span className="text-[#34C759]">{item.successRate}%</span>
                        ) : (
                          <span className="text-[#86868B] font-medium">—</span>
                        )}
                        <span className="text-[10px] text-[#86868B] font-medium block">
      ...
      {topLeader && (
        <p className="text-[10px] text-[#86868B] text-center">
          Champion: <span className="font-bold text-[#1D1D1F]">{topLeader.tech.name}</span> · click any row for the full drill-down.
        </p>
      )}
    </div>
  );

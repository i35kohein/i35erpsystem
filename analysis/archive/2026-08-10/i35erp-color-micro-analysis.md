...
| F5 | P3 | Consolidate: one theme system (either the coat or the CSS themes, not both) | ✅ DONE (commit `b25e50a`): 2 themes (minimalist-clean=light carbon + dark-slate); `.basic-ui` vars now ALIAS the carbon tokens (one source); nunito-navy/apple-clean removed (presets+CSS); 125 legacy hex classes tokenized (bg-brand/bg-surface/border-line/text-ink/text-muted); dark-slate overrides `--color-*` tokens → modern token classes go dark too (was light cards in dark). Verified: light resolves carbon (#0f62fe/#161616/#f4f4f4), dark resolves #131b2e/#f8fafc/#090d16 |

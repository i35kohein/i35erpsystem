...
## F8 — whole-project sweep + modal fixes (2026-08-06 23:xx)
- Full re-sweep after concurrent refactor (carbon removal, Input migration, Create Ticket drawer): all 11 tabs × light/dark = 0 violations, 0 console errors.
- **TicketDetailInspectorModal** (shared by pipeline/pos/qa/intake — was never modal-open audited) had 24 nodes: section-tab buttons (default variant → muted-on-blue 1.23), Close button bg-[var(--primary)] (dark #38BDF8 + white = 2.2), dl/dt/dd misuse ×17, duplicate <main> ×2 (landmark), aria-label on role-less div, scrollable regions, aside landmark-unique. ALL FIXED → 0.
- **bg-[var(--primary)] + text-white pattern** (dark --primary #38BDF8 fails): fixed in CustomDropdownMenu selected option, AI assistant modal (Bot chip, selected suggestion, send), intake icon hover. → bg-brand (dark-stable).
- Remaining: main chunk 992KB (perf), ~62 bg-[#F8F9FA] raw hexes (functional via dynamic CSS, token-policy nice-to-have), Parts Inventory dark 1 intermittent contrast node (toggle-state dependent, passes on re-check).
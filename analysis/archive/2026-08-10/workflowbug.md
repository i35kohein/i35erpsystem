...
| ~~3~~ | ~~🟠 Medium~~ | ~~`server.ts` `/api/auth/login`~~ | ✅ **FIXED 2026-08-10 (commit `4e9ba1a`)** — per-IP rate limit (5/min window), lockout after 10 failures (15 min), constant-time compare (`timingSafeEqual`), tokens stored SHA-256 hashed with on-load migration of legacy raw tokens. Live-verified: 401×5 → 429, correct creds also blocked while limited, all 126 legacy sessions migrated. Env-tunable: `LOGIN_RATE_WINDOW_MS`, `LOGIN_MAX_ATTEMPTS`, `LOGIN_LOCKOUT_MS`, `LOGIN_LOCKOUT_THRESHOLD`. | rate limit + lockout + hashed tokens |
| ~~4~~ | ~~🟡 Low~~ | ~~`server.ts` `/api/gemini/draft-message`~~ | ✅ **FIXED 2026-08-10 (commit `4e9ba1a`)** — prompt now formats `Number(totalCost).toLocaleString() + " MMK"`. | `totalCost.toLocaleString() + " MMK"` |
| ~~5~~ | ~~🟡 Low~~ | ~~`server.ts` SPA fallback~~ | ✅ **FIXED 2026-08-10 (commit `4e9ba1a`)** — `app.use('/api', …)` JSON 404 added before static/SPA fallback (all methods). Live-verified: `GET /api/nonexistent` → `404 {"success":false,…}`. | `app.use('/api', 404 JSON)` |
| ~~6~~ | ~~🟡 Low~~ | ~~`server.ts` static handler~~ | ✅ **FIXED 2026-08-10 (commit `4e9ba1a`)** — `decodeURIComponent` wrapped in try/catch, falls back to raw path. Live-verified: malformed `%zz` no longer 500s (400 at Caddy/Node boundary). | try/catch safe decode |
| ~~7~~ | ~~🟡 Low~~ | ~~QA modal / photos~~ | ✅ **FIXED 2026-08-10 (commit `4e9ba1a`)** — photo caps: QA before/after 4 per set (inline warning notice), New Intake 6 total (toast). Compressed 900px/0.62 keeps each photo ~40–150KB → worst case ~1.2MB, safely under request limits. | cap photo counts + keep downscale |
| ~~8~~ | ~~🟡 Low~~ | ~~`App.tsx` + `auth-tokens.json`~~ | ✅ **FIXED 2026-08-10 (commit `4e9ba1a`)** — `POST /api/auth/logout-all` revokes every session except the caller (no token = all); Admin-only "Sign Out All Devices" button in Settings → User Roles & Permissions. Live-verified: revoked 4, current session kept. | logout-all endpoint + admin UI |
| ~~9~~ | ~~🟢 Info~~ | ~~Telemetry~~ | ✅ **FIXED 2026-08-10 (commit `4e9ba1a`)** — `POST /api/error-log` (JSONL, capped 1000 lines) + client capture in `main.tsx` (window error + unhandledrejection, batched/debounced 3s, fire-and-forget). | error log endpoint + global capture |
| ~~10~~ | ~~🟢 Info~~ | ~~Telegram bot~~ | ✅ **FIXED 2026-08-10 (commit `4e9ba1a`)** — model via `TELEGRAM_AI_MODEL` env; on primary-model failure retries once with provider default (OpenRouter → `openai/gpt-4o-mini`) so a bad pinned model can't break the bot. | env model + fallback chain |
| ~~11~~ | ~~🟢 Info~~ | ~~`SimpleTicketCreator`~~ | ✅ **FIXED 2026-08-10 (commit `4e9ba1a`)** — auto-print guarded (try/catch) + explicit 🖨 Print Ticket button in the saved flash as fallback. Live-verified: flash shows Print button after save. | guarded print + explicit button |
| ~~12~~ | ~~🟢 Info~~ | ~~QA gate~~ | ✅ **FIXED 2026-08-10 (commit `4e9ba1a`)** — "Reopen QA" (RotateCcw) on Finished tickets with a passed checklist (Work Intake roster + Ticket Board): confirm → clears `postRepairChecklist` + `afterRepairPhotos` → ticket flows back into QA queue for re-inspection. Live-verified end-to-end: Diagnose → QA Pass → Reopen QA → back in QA queue. | "Reopen QA" for Finished tickets |
